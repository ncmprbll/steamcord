        ...
        {#if signin && signupOutroend}
            <div transition:fade={{ duration: duration }} on:outroend={toggle} class="box">
                <div class="logo">
                    ...
                    <span>{$page.data.localization.loginTitle}</span>
                </div>
                <form method="POST" action="/api/auth/login" class="form" on:submit|preventDefault={handleLogin}>
                    <div class="box-input">
                        ...
                        {#if code === 404}
                            <span class="input-message">{$page.data.localization.badCredentials}</span>
                        {/if}
                    </div>
                    <div class="box-input">
                ...
                <div class="separator">
                    <span>{$page.data.localization.noaccount}</span>
                </div>
                <div class="form">
                    <button class="form-button" on:click={() => {signin = !signin}}>{$page.data.localization.createAccount}</button>
                </div>
            </div>
        {:else if !signin && signinOutroend}
            <div transition:fade={{ duration: duration }} on:outroend={toggle} class="box">
                <div class="logo">
                    ...
                    <span>{$page.data.localization.signupTitle}</span>
                </div>
                <form method="POST" action="/api/auth/register" class="form" on:submit|preventDefault={handleRegister}>
                    <div class="box-input">
                        ...
                        {#if code === 409}
                            <span class="input-message">{$page.data.localization.userAlreadyExists}</span>
                        {/if}
                    </div>
                    <div class="box-input">
                        <label for="email">{$page.data.localization.email}</label>
                    ...
                    <div class="box-input">
                        ...
                        {#if code === -1}
                            <span class="input-message">{$page.data.localization.passMismatch}</span>
                        {/if}
                    </div>
                    <div class="box-input">
                        <label for="confirm">{$page.data.localization.confirmPass}</label>
                    ...
                    <button class="form-button" type="submit">
                        {#if loading}
                            <Spinner size="16"/>
                        {:else}
                            {$page.data.localization.signup}
                        {/if}
                    </button>
                </form>
                <div class="separator">
                    <span>{$page.data.localization.yesaccount}</span>
                </div>
                <div class="form">
                    <button class="form-button" on:click={() => {signin = !signin}}>{$page.data.localization.backToLogin}</button>
                </div>
            </div>
        {/if}
    </div>
