        ...
        {#if signin && signupOutroend}
            <div transition:fade={{ duration: duration }} on:outroend={toggle} class="box">
                <div class="logo">
                    ...
                    <span>{$page.data.locale.loginTitle}</span>
                </div>
                ...
                <div class="separator">
                    <span>{$page.data.locale.noaccount}</span>
                </div>
                <div class="form">
                    <button class="form-button" on:click={() => {signin = !signin}}>{$page.data.locale.createAccount}</button>
                </div>
            </div>
        {:else if !signin && signinOutroend}
            <div transition:fade={{ duration: duration }} on:outroend={toggle} class="box">
                <div class="logo">
                    ...
                    <span>{$page.data.locale.signupTitle}</span>
                </div>
                <form method="POST" action="/api/auth/register" class="form" on:submit|preventDefault={handleRegister}>
                    <div class="box-input">
                        ...
                        {#if code === 409}
                            <span class="input-message">{$page.data.locale.userAlreadyExists}</span>
                        {/if}
                    </div>
                    <div class="box-input">
                        <label for="email">{$page.data.locale.email}</label>
                    ...
                    <div class="box-input">
                        ...
                        {#if code === -1}
                            <span class="input-message">{$page.data.locale.passMismatch}</span>
                        {/if}
                    </div>
                    <div class="box-input">
                        <label for="confirm">{$page.data.locale.confirmPass}</label>
                    ...
                    <button class="form-button" type="submit">
                        {#if loading}
                            <Spinner size="16"/>
                        {:else}
                            {$page.data.locale.signup}
                        {/if}
                    </button>
                </form>
                <div class="separator">
                    <span>{$page.data.locale.yesaccount}</span>
                </div>
                <div class="form">
                    <button class="form-button" on:click={() => {signin = !signin}}>{$page.data.locale.backToLogin}</button>
                </div>
            </div>
        {/if}
    </div>
