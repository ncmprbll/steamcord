...
func (s *Repository) GetTier(ctx context.Context, limit string) ([]*models.TierRow, error) {
	const query = `
				SELECT products.id, products.name, products.discount, json_object_agg(products_prices.currency_code, json_build_object('price', products_prices.price, 'symbol', currencies.symbol)) as prices, products_images.tier_background_img
					FROM products
...
func (s *Repository) GetFeatured(ctx context.Context) ([]*models.FeaturedRow, error) {
	const query = `
				SELECT products.id, products.name, products.discount, json_object_agg(products_prices.currency_code, json_build_object('price', products_prices.price, 'symbol', currencies.symbol)) as prices, products_images.featured_background_img, products_images.featured_logo_img, json_agg(products_platforms.platform) as platforms
				FROM products
